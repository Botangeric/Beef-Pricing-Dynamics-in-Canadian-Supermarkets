...
**Old price**:This variable represents the product's previous price. It serves as an indicator of discounts or price changes, which are key factors in determining the current_price. It provides a direct measure of pricing adjustments over time.
...
The model is a Bayesian linear regression that predicts the current price of beef products. It uses the scaled old price old price as a numerical variable and includes vendor and month as categorical variables. This setup allows the model to account for the effects of historical prices, differences between supermarkets, and seasonal variations.
...
- $\beta_3$ The coefficient for month, representing the effect of different months on the current price (a categorical variable encoded as dummy variables).
...
In this setup, the primary expense will be software development to automate the data collection and storage process. This system will help streamline data gathering and ensure consistency over time. However, all activities must be conducted with the explicit consent of the supermarkets and in full compliance with ethical and legal standards to ensure no infringement on the rights of individuals or groups.This way, we can obtain three pieces of data: the product name, where it is sold, and its price.We can also collaborate with these supermarkets to obtain their beef product pricing information directly.
